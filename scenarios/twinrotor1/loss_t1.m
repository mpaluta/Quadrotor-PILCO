%% loss_cp.m
% *Summary:* twinrotor1 loss function; the loss is 
% $1-\exp(-0.5*d^2*a)$,  where $a>0$ and  $d^2$ is the squared difference
% between the actual and desired position of tip of the rotor. 
...
function [L, dLdm, dLds, S2] = loss_t1(cost, m, s)
%% Code
...
% disp(['d0 = ',num2str(D0)])
% disp(['dimensions of mean vector = ',num2str(size(m))]);
% disp(['dimensions of covariance matrix = ',num2str(size(s))]);
