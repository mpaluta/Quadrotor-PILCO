%% settings_cp.m
% *Summary:* Script set up the twinrotor1 scenario
%
...
dyni = [2 3 4];              % variables that serve as inputs to the dynamics GP
poli = [2 3 4];              % variables that serve as inputs to the policy
...
% 3. Plant structure
plant.dynamics = @dynamics_t1;                    % dynamics ode function
...
policy.p.targets = 0.1*randn(nc, length(policy.maxU));    % init. policy targets 
                                                          % (close to zero)
policy.p.hyp = repmat(log([1 0.7 0.7 1 0.01])',1,2);      % initialize policy
                                                          % hyper-parameters

% 5. Set up the cost structure
cost.fcn = @loss_t1;                       % cost function
